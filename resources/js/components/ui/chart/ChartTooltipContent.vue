...
const payload = computed(() => {
  // 1. Cek apakah ada nameKey (misal: "type") dan ambil valuenya (misal: "masuk")
  const categoryKey = props.nameKey
  const categoryValue = categoryKey ? props.payload[categoryKey] : null

  // 2. Ambil config berdasarkan value kategori tadi (cari config "masuk")
  const categoryConfig = categoryValue ? props.config[categoryValue] : null

  // 3. Loop semua data di payload
  return Object.entries(props.payload)
    .map(([key, value]) => {
       // HAPUS key "type" dari tampilan (karena kita cuma mau angkanya)
       if (key === categoryKey) return null

       // HAPUS jika value bukan angka (biar aman dari sampah data lain)
       if (typeof value !== 'number') return null

       // Ambil config asli milik "count" (kalau ada)
       const originalConfig = props.config[key] || {}

       // --- LOGIC PAKSA (OVERRIDE) ---
       // Prioritas Label: 1. Config "Masuk" | 2. Config "Karyawan" | 3. Nama Key "count"
       const label = categoryConfig?.label || originalConfig.label || key

       // Prioritas Warna: 1. Config "Masuk" | 2. Config "Karyawan" | 3. Warna Default
       const color = categoryConfig?.color || originalConfig.color || props.payload.fill

       return {
         key,
         value,
         itemConfig: { ...originalConfig, label, color },
